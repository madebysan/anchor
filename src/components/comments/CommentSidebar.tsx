...
import { MessageCircle, MessageSquare, Plus, ArchiveRestore, Sparkles } from "lucide-react";
...
interface CommentSidebarProps {
  ...
  onSubmitChatMessage?: (text: string) => void;
...
export default function CommentSidebar({
  ...
  const [mode, setMode] = useState<"comments" | "chat">("comments");
  const [showResolved, setShowResolved] = useState(false);
  const commentThreads = threads.filter((t) => t.intent !== "chat");
  const chatThread = threads.find((t) => t.intent === "chat" && t.status === "active");
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      />
      <Tabs value={mode} onValueChange={(value) => setMode(value as "comments" | "chat")} className="flex-1 min-h-0 gap-0">
            ...
            <div className="p-3 space-y-2">
              ...
              {activeThreads.length === 0 && resolvedThreads.length === 0 && (
                <div className="flex flex-col items-center justify-center text-center px-6 py-16">
                  <MessageSquare className="h-10 w-10 text-muted-foreground/40 mb-3" aria-hidden="true" />
        ...
        <TabsContent value="chat" className="min-h-0">
          <DocumentChat
            thread={chatThread}
            onSubmitMessage={(text) => onSubmitChatMessage?.(text)}
            onRevertAppliedEdit={onRevertAppliedEdit}
            isLoading={chatIsLoading}
            onStopGeneration={
              chatThread && onStopGeneration
                ? () => onStopGeneration(chatThread.id)
                : undefined
            }
            triggerOptions={triggerOptions}
            triggerConfigs={triggerConfigs}
